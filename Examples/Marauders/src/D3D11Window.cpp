...
{
	if (DX11_Base::g_GameData->m_ShowMenu)
		return FALSE;
...
namespace DX11_Base {
	...
	bool D3D11Window::InitWindow()
	{
		...
		WindowHwnd = CreateWindow(WindowClass.lpszClassName, L"JSdsjkafbla", WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, WindowClass.hInstance, NULL);
		if (WindowHwnd == NULL) {
			return FALSE;
		}
#if DEBUG
	...
	{
		SetWindowLongPtr(g_GameData->g_GameWindow, GWLP_WNDPROC, (LONG_PTR)m_OldWndProc);
		DisableAll();
		return;
	}
